<template>
  <div class="users">
    ...
    <div style="margin-top:10px;padding-left:30px">
      用户名 <el-input v-model="input" placeholder="请输入用户名" style="width:250px"></el-input>
      <el-button type="success" style="margin-left:30px">查询</el-button>
      <el-button type="primary" style="margin-left:30px">新增</el-button>
    </div>
    <div>
      <el-table v-loading="loadingStatus" :data="userList" border :cell-style="{ textAlign: 'center' }"
        ...
        <el-table-column label="状态">
          <template v-slot="scope">
            <el-switch v-model="scope.row.status" active-color="#13ce66" :active-value="1" :inactive-value="2">
            </el-switch>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="250px">
          <template v-slot="scope">
            ...
            <el-button size="mini" :disabled="scope.row.roles.length === 1 && scope.row.roles[0].code === 'admin'"
              type="warning" @click="handleOpenRoleDialog(scope.row)" plain>分配角色</el-button>
            <el-button size="mini" :disabled="scope.row.roles.length === 1 && scope.row.roles[0].code === 'admin'"
      ...
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="current"
        :page-sizes="[2, 5, 10]" :page-size="size" layout="jumper,  prev, pager, next,sizes,  total" :total="total">
      </el-pagination>
